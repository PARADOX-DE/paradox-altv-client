...
export default class AnimationHandler {
    ...
    play() {
        if(game.hasAnimDictLoaded(this.dict)) game.taskPlayAnim(this.scriptId, this.dict, this.name, 1, -1, this.duration, this.flag, 1.0, false, false, false);
        else this.loadDict(this.dict).then(success => {
            if(success) game.taskPlayAnim(this.scriptId, this.dict, this.name, 1, -1, this.duration, this.flag, 1.0, false, false, false);
        });
