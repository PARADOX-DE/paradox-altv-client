...
export class PhoneView extends Window {
    ...
    constructor() {
        ...
        EventController.onServer("ResponseOpenPhone", this.openPhone.bind(this));
    }
    ...
    playAnimation(status: string, freeze: boolean, force: boolean) {
        ...
        return new Promise(async resolve => {
            ...
            game.taskPlayAnim(alt.Player.local.scriptID, dict, animation, 8, -4, -1, freeze ? 14 : 50, 0, false, false, false);
    ...
    onKey(key: number, down: boolean) {
        if(down && key == 112) return EventController.emitServer("RequestOpenPhone");
    }
}
