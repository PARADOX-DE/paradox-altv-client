...
alt.everyTick(() => {
    game.setWeaponDamageModifierThisFrame(0xA2719263, 0.3)
    for (const hash of melee) {
        game.setWeaponDamageModifierThisFrame(hash, 0.2)
    }

    game.setPedUsingActionMode(alt.Player.local.scriptID, false, 1, "DEFAULT_ACTION")

    if (game.isPedArmed(alt.Player.local.scriptID, 6)) {
        game.disableControlAction(alt.Player.local.scriptID, 140, true) // INPUT_MELEE_ATTACK_LIGHT
